...
* Automated creation of Level Requests using a Scripting Tool. A scripting tool is used to define the triggers (e.g., server backup) for Level Request creation. This guide describes the use of an <<CommandLine.adoc#_sautomatecreatinglevelrequestswithant,Automating>>. Refer to the indicated section.
...
Refer to the <<Desktop_LevelRequests.adoc#_desktop_lr_overview,Level>>in the web application to verify if the Level Request was _executed_ successfully as well.
--
