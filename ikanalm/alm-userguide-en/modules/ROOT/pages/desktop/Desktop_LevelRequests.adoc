...
The Notification type (mail, netsend) and the Notification criteria (if Level Request Status is SUCCESS, FAIL, WARNING or ALWAYS) are defined in the sections <<_plevelenvmgt_createbuildlevel>>, <<_plevelenvmgt_createtestorproductionlevel>> or <<_plevelenvmgt_editlevel>>.
|===
...
+
For a detailed description of this screen refer to <<_desktop_lr_overview>>.
...
If the previous Level in the Lifecycle is marked as _Optional_ (<<_lifecycles_makingoptional>>) the list contains the available Builds from the previous Level AND from the Level before that one.
...
[NOTE]
====
If the__ Auto Refresh __option is activated, the __Level Requests Overview __screen will be refreshed each time the defined rate is expired. <<_desktop_autorefresh>>
====
...
+
For more information on the usage of search panels and filters, refer to the sections <<_babcjedaj8>> and <<_desktop_searchfilters>>.
...
[NOTE]
====
If the__ Auto Refresh __option is activated, the _Level Requests Overview_ will be refreshed following the interval specified by the Auto Refresh Rate specified in the System Settings. <<_desktop_autorefresh>>
====
...
For more information, refer to the section explaining the <<_desktop_lr_summary>> tab page of the _Level Request Detail_ screen. 
...
For a description of the latter four Level Request Action Types, refer to <<_desktop_lr_creatinglevelrequest>>.
...
History Detail_ screen. <<_desktop_lr_buildhistory>>
...
For more information, refer to the section <<_desktop_lr_sources>>.
...
+
For more information, refer to the section <<_dekstop_lr_detailedoverview>>.
...
+
For more detailed information, refer to the section <<_desktop_lr_buildhistory>>.
...
+
For more information on the _Auto Refresh_ settings, refer to the section <<_desktop_autorefresh>>.
...
* <<_desktop_lr_summary_buildsdeploys>>
* <<_desktop_lr_summary_approvals>>
* <<_desktop_lr_summary_issues>>
* <<_desktop_lr_summary_errorwarning>>
...
[NOTE]
====
For a complete list of all Approvals defined for the Level Request, select the _Approvals_ tab underneath the Status header. <<_desktop_lr_approvals>>[[_desktop_lr_summary_issues]]
====
...
For more information on the _URL_ field, refer to the section <<_globadm_issuetrackingcreate>>.
...
If you want to edit, delete or synchronize them, select the _Issues_ tab underneath the Status header. <<_desktop_lr_issues>>[[_desktop_lr_summary_errorwarning]]
====
...
Logs_ tab page (<<_desktop_lr_phaselogs>>).
...
For more information on specifying the format of the log, refer to the section <<_globadm_scriptingtools>>.
|===
...
For a more detailed description, refer to the section <<_phases_levelphases>>.
====
...
Refer to <<_globadm_issuetrackingcreate>>for more information on the URL field.
...
* image:images/icons/edit.gif[,15,15] __ Edit__. <<_desktop_lr_issueedit>>
* image:images/icons/delete.gif[,15,15] __ Delete__. <<_desktop_lr_issuedelete>>
* image:images/icons/icon_synchronize.png[,15,15] __ Synchronize__. <<_desktop_lr_issuesynchronize>>
* __Add Issue__. <<_desktop_lr_issueadd>>
====
...
+
For a detailed description of the fields, refer to <<_desktop_lr_issueadd>>.
...
After having created the Issue, synchronize the issue (<<_desktop_lr_issuesynchronize>>) to automatically retrieve the additional fields from the Issue Tracking System.
====
...
+
If the__ Auto Refresh __option is activated, the _Build History_ screen will be refreshed each time the defined rate is expired. <<_desktop_autorefresh>>
+
...
[NOTE]
====
If the__ Auto Refresh __option is activated, the _Build and Deploys Overview_ screen will be refreshed each time the defined rate is expired. <<_desktop_autorefresh>>
====
...
For more information, refer to the section <<_desktop_lr_summary>>.
