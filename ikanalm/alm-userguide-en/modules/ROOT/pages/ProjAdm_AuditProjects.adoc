...
+
image::images/ProjAdmin-Audit-Success.png[,964,610] 
+
If there are errors in the Project configuration, the following screen is displayed:
+
image::images/ProjAdmin-Audit-Error.png[,1037,573] 
+
Locked Project Streams and Levels have a image:images/icons/locked.gif[,15,15] _ Locked_ icon in the _Locked_ field.
...
+
For the list of possible error and warning messages, refer to the section <<ProjAdm_AuditProjects.adoc#_projadm_auditingprojects_errorswarnings,id==_projadm_auditingprojects_errorswarnings--title==Auditing Projects id==_projadm_auditingprojects_errorswarnings--title==Error and Warning Messages>>.
...
** <<ProjAdm_AuditProjects.adoc#_babgjghf,id==_babgjghf--title==Error Messages>>
** <<ProjAdm_AuditProjects.adoc#_babcfbhf,id==_babcfbhf--title==Warning Messages>>
...
Here you may set a value for the alm.phase.builder parameter by clicking the _Edit_ link (<<#_globadm_phaseparameters_overview,>>).
|===
