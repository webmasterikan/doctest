...
These locations are automatically synchronized with the built-in Local FileCopy transporter. <<_globadm_transporter_filecopy>>
...
For more information, refer to the sections <<_globadm_transporters>>and <<_globadm_machines>>.
...
IKAN ALM Users_ submenu option. <<_globadm_notifyusers>>
====
...
|Auto Refresh Rate
|In this field, you can enter the number of seconds for the automatic refresh function on the <<Desktop_ManageDesktop.adoc#_managedesktop_desktopoverview>>, <<Desktop_LevelRequests.adoc#_desktop_lr_overviewscreen>> and <<_desktop_lr_buildhistory>>.
...
This Build Archive location is defined in the _Local Environment_ tab of the System Settings. <<_globadm_systemsettings_localenvironment>>
====
+
...
|View Content
|Click the image:images/icons/view.gif[,15,15] _View_ link to view the content of the Build File. <<_desktop_lr_results>>
