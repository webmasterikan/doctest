...
You can check the installed Phases using the _Installed Phases_ link next to the IKAN ALM Server Machine on the __Machines Overview__. <<_globadm_machinesoverview>>
...
Overview__. <<_globadm_machinesoverview>>


The relative locations for all remote transporters must be defined in the _System Settings_ section. <<_globadm_system_settings>>

[NOTE]
====
A Transporter will be activated by linking the Transporter to a Machine representing the IKAN ALM Agent (<<_globadm_machinesoverview>>), and by linking that Machine to a Build or Deploy environment (<<_pcreatebuildenvironment>> or <<_pcreatedeployenvironment>>).
====
...
A FileCopy Transporter makes it possible to use "`file copy`" for transporting Custom Phases, files and directories between the IKAN ALM server and a local or remote Agent handling the Build or Deploy processes (see the introduction on <<_globadm_transporters>>).
...
FileCopy Locations_ and can only be updated in the System Settings section. <<_globadm_system_settings>>
...
|Enter the prefix for the Build Archive Location (the location where the Builds are stored or retrieved from by an IKAN ALM Agent handling a Build or Deploy process). The Server Location will be formed by concatenating this prefix together with the Relative Location defined in the System Settings. <<_globadm_system_settings>>
...
The Server Location will be formed by concatenating this prefix together with the Relative Location defined in the System Settings. <<_globadm_system_settings>>
...
_Note:_ The value entered in this field will override the value specified in the System Settings. <<_globadm_system_settings>>If not specified in the System Settings nor on this screen, the default FTP Port Number `21` will be used.
...
|Build Archive Path Prefix
|Enter the prefix for the Build Archive Location (the location where the Builds are stored or retrieved from). The Server Location will be formed by concatenating this prefix together with the Relative Location defined in the System Settings. <<_globadm_system_settings>>
...
|Script Path Prefix
|Enter the prefix for the Script Location (the location where the IKAN ALM Agent will retrieve the build or deploy script from if it cannot be found in the sources). The Server Location will be formed by concatenating this prefix together with the Relative Location defined in the System Settings. <<_globadm_system_settings>>
...
_Note:_ The value entered in this field will overrule the value specified in the System Settings. <<_globadm_system_settings>>If not specified in the System Settings nor on this screen, the default SSH Port Number `22` will be used.
